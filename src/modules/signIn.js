...
const SET_LOCATION = 'signIn/SET_LOCATION';
...
const signIn = handleActions(
  {
    [CLICK_SIGN_IN_BTN]: (state, action) => ({
      ...state,
    ...
    [SET_EMAIL_ERROR]: (state, { payload: emailErrorMsg }) => ({
      ...state,
      emailErrorMsg,
    }),
    [SET__PASSWORD_ERROR]: (state, { payload: passwordErrorMsg }) => ({
      ...state,
      passwordErrorMsg,
    }),
    [SET_SIGN_IN_ERROR]: (state, { payload: signInErrorMsg }) => ({
      ...state,
      signInErrorMsg,
    }),
    [ERASE_EMAIL_ERROR]: (state, action) => ({ ...state, emailErrorMsg: null }),
    [ERASE_PASSWORD_ERROR]: (state, action) => ({
      ...state,
      passwordErrorMsg: null,
    }),
    [ERASE_SIGN_IN_ERROR]: (state, action) => ({
      ...state,
      signInErrorMsg: null,
    }),
    [CHANGE_LOGGED_IN_STATE]: (state, action) => ({
      ...state,
      isLoggedIn: !state.isLoggedIn,
    }),
    [SET_PET_NAME]: (state, { payload: petName }) => ({ ...state, petName }),
    [SET_PET_BREED]: (state, { payload: petBreed }) => ({ ...state, petBreed }),
    [SET_LOCATION]: (state, { payload: locationName }) => ({
      ...state,
      locationName,
    }),
  },
